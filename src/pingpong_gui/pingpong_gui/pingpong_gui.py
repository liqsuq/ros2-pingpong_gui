...
class Canvas(FigureCanvasQTAgg):
    def __init__(self, parent=None, figwidth=400, figheight=400, dpi=100):
        ...
        self.axes = fig.add_subplot(111, xlim=(0.0, 1000.0), ylim=(0.0, 1000.0))
        self.axes.set_title("Topic ping-pong latency", fontsize=18)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self, args=None, parent=None, width=600, height=800):
        ...
        proc = subprocess.run("which run", shell=True, capture_output=True)
        if proc.returncode:
            command = "taskset -c " + str(self.nproc-3)
        else:
            command = "run -s rr -P98 -b " + str(self.nproc-3)
        self.process_ping = QtCore.QProcess()
        self.process_ping.start(command + " ros2 run pingpong ping")
        if proc.returncode:
            command = "taskset -c " + str(self.nproc-4)
        else:
            command = "run -s rr -P98 -b " + str(self.nproc-4)
        self.process_pong = QtCore.QProcess()
        self.process_pong.start(command + " ros2 run pingpong pong")
    ...
    def closeEvent(self, event):
        if self.process_ping.state() != QtCore.QProcess.NotRunning:
            subprocess.run("killall ping", shell=True)
        if self.process_pong.state() != QtCore.QProcess.NotRunning:
            subprocess.run("killall pong", shell=True)
    ...
    def initUI(self, width, height):
        ...
        self.setWindowTitle('pingpong_gui')
        # Layouts
        ...
        line2d = self.canvas.axes.plot(xdata, ydata, "r.", markersize=1)
        self.lines = line2d[0]
    ...
    def on_toggled_test(self, checked):
        if checked:
            ...
            self.process_test.start("ros2 run pingpong logger")
        else:
        ...
        if checked:
            ...
            if proc.returncode:
                ...
                command = "pkexec shield -a +" + \
                          str(self.nproc - 4) + "-" + str(self.nproc - 3)
